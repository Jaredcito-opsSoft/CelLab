...
export function CashView({ token, role, business }: { token: string; role: Role; business: Business | null }) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const [currentRes, sessionsRes, today, week, month] = await Promise.all([
        apiRequest<{ item: CashSession | null }>('/api/operations/cash/current', {}, token),
        apiRequest<{ items: CashListItem[] }>('/api/operations/cash/sessions?limit=30', {}, token),
  ...
  async function open(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await apiRequest<{ item: CashSession }>('/api/operations/cash/open', { method: 'POST', body: JSON.stringify({ openingCashCents: cents(form.get('openingCash')), notes: form.get('notes') || null }) }, token);
  ...
  async function manual(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget);
    try {
      await apiRequest('/api/operations/cash/manual-movement', { method: 'POST', body: JSON.stringify({ type: form.get('type'), method: form.get('method'), amountCents: cents(form.get('amount')), reason: form.get('reason'), note: form.get('note') || null }) }, token);
  ...
  async function close(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget);
    try {
      const res = await apiRequest<{ item: CashSession }>('/api/operations/cash/close', { method: 'POST', body: JSON.stringify({ countedCashCents: cents(form.get('countedCash')), notes: form.get('notes') || null }) }, token);
